#develop metadata on each hour of each data for the data set
hourlyMetrics <- function(data) {
  ...
  if (TRUE %in% sapply(events, is.null)) {
    events <- events[-which(sapply(events, is.null))]
  }
  
  events <- events[sapply(events, function(x) dim(x)[1]) > 0]; print(head(events)) 
