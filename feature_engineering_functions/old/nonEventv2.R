...
nonEvent <- function(data, threshold, event_duration) {
  ...
  while (index <= max(threshold_data$index)) {
    ...
    above_threshold <- subset(non_threshold_data, secondspastepoch >= the_timestamp & secondspastepoch <= 
    ...
    if (nrow(above_threshold)==0) { 
      ...
      temp <- subset(threshold_data, index > max(above_threshold$index))
      index <- min(temp[,'index']) #and set it as the new index
    }
  }
